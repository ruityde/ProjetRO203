...
function timeVsSizeDiagram(outputFile::String)
    ...
    # Array that will contain the resolution times (one line for each subfolder)
    results = Array{Tuple{Int64, Float64}}(undef, subfolderCount, maxSize)
    ...
    for file in readdir(resultFolder)
        ...
        if isdir(path)
            ...
            for resultFile in filter(x->occursin(".txt", x), readdir(path))
                ...
                if isOptimal
                    results[folderCount, fileCount] = (n,solveTime)
    ...
    for dim in 1: size(results, 1)
        ...
        while currentId <= size(results, 2) && results[dim, currentId][2] != Inf
            ...
            # Number of elements with a size of n
            identicalValues = 0
        ...
        if dim == 1
            ...
            # Draw a new plot
            plot(x, y, label = folderName[dim], legend = :bottomright, xaxis = "Instance size", yaxis = "Time (s)",linewidth=3)
